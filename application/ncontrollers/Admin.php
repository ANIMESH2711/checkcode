...
/*
	...
	{
		$data['rs']  = $this->lib_model->Select('m_role', 'id,name,role', array('status' => 0));
		$data['rs2'] = $this->lib_model->Select('m_faculty', 'id,name,email', array('status' => 0));
		$this->load->view('admin/header', $data);
...
/*
	...
	public function F_add_roles_process()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$count = $this->lib_model->Counter('m_role_assignment', array('fid' => $F,'sid'=>$S));
			if ($count == 0) {
				$f = array(
					'fid' => $F,
					'sid' => $S,
				...
				$this->lib_model->Insert('m_role_assignment', $f );
				$Msg = array('Msg' => 'Mapping Successfully done', 'Type' => 'success');
				$this->session->set_flashdata($Msg);
				redirect(base_url('admin/F_add_roles/' ));
			} else {
